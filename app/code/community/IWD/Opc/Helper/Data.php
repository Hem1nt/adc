...
class IWD_Opc_Helper_Data extends Mage_Core_Helper_Abstract{
	...
	public function getValues($zipCode) {

		$csvObject= new Varien_File_Csv();
		$zipcodefile = Mage::getStoreConfig('custom_snippet/zipcode_upload/upload');
		$filePath = Mage::getBaseDir('media').'/zipcode/'.$zipcodefile;
		// $filePath = Mage::getUrl('media/zipcode').''.$zipcodefile;

		try {
			$fileContent = $csvObject->getData($filePath);
			foreach ($fileContent as $rowContent){
				if($rowContent[0] == $zipCode['pincode']){
					$city  = str_replace('_',' ',uc_words($rowContent[1]));
					$state = str_replace('_',' ',uc_words($rowContent[2]));
					break;
				}
			}
		}catch (Exception $e) {
			echo 'Csv: ' . $filePath . ' - getCsvData() error - '. $e->getMessage();
		}
