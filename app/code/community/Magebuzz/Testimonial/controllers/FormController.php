...
class Magebuzz_Testimonial_FormController extends Mage_Core_Controller_Front_Action {
	...
	const XML_PATH_EMAIL_SELECT_TEMPLATE_AFTER_ADMIN = 'testimonial/email_configuration/select_template_admin';
	...
	public function save() {
		...
		if ($post) {
			...
			try {
				...
				//$model->save();
				$model->setData('rating',$post['rating'])->save();

				if (Mage::getStoreConfig('testimonial/email_configuration/send_email_after_post_testimonial', Mage::app()->getStore())=="1") {
					$admin_email = 'cs@alldaychemist.com';
					$admin_user_name = 'Admin';
					$loginStatus = Mage::getSingleton('customer/session')->isLoggedIn();
					if($loginStatus){
						$to_email = Mage::getSingleton('customer/session')->getCustomer()->getEmail();
					}
					else
					{
						$to_email=$this->getRequest()->getPost('email');
						$user_name=$this->getRequest()->getPost('name');
					}
					$to=array('email'=>$to_email, 'name'=>$user_name);
					$admin_to=array('email'=>$admin_email, 'name'=>$admin_user_name);
					$this->sendemailAdminAction($admin_to, $templateConfigPath=self::XML_PATH_EMAIL_SELECT_TEMPLATE_AFTER_ADMIN);
	...
	public function sendemailAdminAction($to, $templateConfigPath) {
		$post=$this->getRequest()->getPost();
		if(!$to) return;
		// print_r($post);exit;
		$translate=Mage::getSingleton('core/translate');
	...
	public function postAction() {
		if(Mage::getStoreConfig('testimonial/testimonial_options/testimonial_captcha_enabled', Mage::app()->getStore())==true) {
			...
			if ($code !=  $captcha_code) {
				Mage::getSingleton('core/session')->setTestimonial($post);
