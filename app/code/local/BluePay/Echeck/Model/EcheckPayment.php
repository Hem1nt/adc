...
//const CGI_URL = 'https://secure.bluepay.com/interfaces/bp10emu';
    ...
    public function authorize(Varien_Object $payment, $amount)
    {
    	...
    	$result = $this->capture($payment,$amount);
...
/*    	$payment->setCcApproval($result->getAuthCode())
    ...
    {	
    	...
    	$request= $this->_buildRequest($payment);
    	$echechresponse  = $this->_echeckResponse($request,$payment);
    	if($echechresponse != "1")
    	{
			//$status_result = $this->_echeckstatus($echechresponse);
    		$status = '0';
    		Mage::getSingleton('core/session')->setTransactionid($echechresponse);
    	}
    	else
    	{
    		$status = '1';
       ...
       if ($result->getMessage()) {
        ...
        Mage::getSingleton('core/session')->setEcheckstatus($status);
		Mage::getSingleton('core/session')->setMyEcheckstatus($status); //@ added by nilesh 
		//echo "Transaction Result:------- ".Mage::getSingleton('core/session')->getEcheckstatus()." ---- ".$status . "<br><br>";exit;
		//print_r(Mage::getSingleton('core/session')->getEcheckstatus());exit;
	}
	public function _echeckResponse(Varien_Object $request,Varien_Object $payment)
	{
		$order = $payment->getOrder();
		
		//echo $order->getIncrementId();exit;
		$order_id = $order->getIncrementId();
		Mage::getSingleton('core/session')->setMyOrderId($order_id);
		$state = $request['state'];
		$sta = $state[0].$state[1];
		Mage::log($state,null,'state.log');
//-----------------------//start theEcheck Payment gateway intigration

					//echo "cvb--".$_SERVER["REMOTE_ADDR"];exit;
					//create array of data to be posted
					//$post_data['security_key'] = '4c61363cd88d59a5c7257390fe991de9';
					$post_data['security_key'] = '4c61363cd88d59a5c7257390fe991de9';
					$post_data['purchaser_firstname'] = $request['firstname'];
					$post_data['purchaser_lastname'] = $request['lastname'];
					$post_data['purchaser_address'] = $request['addr1'];
					$post_data['purchaser_city'] = $request['city'];
					$post_data['purchaser_state'] = $sta;
					$post_data['purchaser_zipcode'] = $request['zipcode'];
					$post_data['purchaser_phone'] = $request['phone'];
					$post_data['purchaser_email'] = $request['email'];
					$post_data['transaction_amount'] = $request['amount'];
					$post_data['purchaser_ip'] = $_SERVER["REMOTE_ADDR"];
					$post_data['purchaser_account'] = $request['account'];
					$post_data['purchaser_routing'] = $request['routing']; // Valid routing number
					//$post_data['transId'] = $order_id; //set oredrid as transactionid 
					//$post_data['get_status'] = '0';
					...
					// print_r(curl_getinfo($curl_connection));exit;
				...
				//-------------------------end theEcheck Payment gateway intigration 				

				}
				public function _echeckstatus($transctionid)
				{
					$post_data['security_key'] = '4c61363cd88d59a5c7257390fe991de9';

					$post_data['transId'] = $transctionid;
					$post_data['get_status'] = '1';
			...
			//echo "Transaction Result: " . $result . "<br><br>";exit;
	...
	protected function _buildRequest(Varien_Object $payment)
	{
		...
		$request->setMode(($this->getConfigData('test_mode') == 'LIVE') ? 'TEST' : 'LIVE');
		...
		if (!empty($order)) {
			...
			$regionId = $billing->getData('region_id');
			if(!empty($regionId)){
			...
			Mage::log($billing->getData(),null,'billing_data.log');
			Mage::log($regionName,null,'billing_data.log');
			if (!empty($billing)) {
       ...
       /* switch ($payment->getPaymentType()) {
		...
		//new code
            ...
            $payment_param = Mage::app()->getRequest()->getParams();
			//$request->setSignature($payment['payment']['signature']);
			//echo "<pre>";
//print_r($payment_param['payment']);exit;
            $request->setFirstname($payment_param['payment']['echeck_firstname']);
            $request->setLastname($payment_param['payment']['echeck_lastname']);
            $request->setRouting($payment_param['payment']['echeck_routing_number']);
            $request->setAccount($payment_param['payment']['echeck_bank_acct_num']);
		//end code
            return $request;
        }
