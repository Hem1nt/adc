<?php
class Excellence_Echeckapi_IndexController extends Mage_Core_Controller_Front_Action
{
		...
		try{
			...
			elseif($request['Status_'] == 90){
				...
				try {
					...
					$message = Mage::helper('echeckapi')->__('Notified customer about invoice #%s.', $invoice->getIncrementId());
					$comment = $order->sendNewOrderEmail()->addStatusHistoryComment($message)
					->setIsCustomerNotified(true)
					->save();
				}
