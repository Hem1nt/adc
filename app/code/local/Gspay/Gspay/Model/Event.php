...
class Gspay_Gspay_Model_Event
{
        ...
        try {
            ...
            switch($params['status']) {
                case self::GSPAY_STATUS_FAIL: //fail
                    $msg = Mage::helper('gspay')->__('Payment failed.');
                    $this->_processCancel($msg);
                    break;
                case self::GSPAY_STATUS_CANCEL: //cancel
                    $msg = Mage::helper('gspay')->__('Payment was canceled.');
                    $this->_processCancel($msg);
                    break;
                case self::GSPAY_STATUS_PENDING: //pending
                    $msg = Mage::helper('gspay')->__('Pending bank transfer created.');
                    $this->_processSale($params['status'], $msg);
                    break;
                case self::GSPAY_STATUS_SUCCESS: //ok
                    $msg = Mage::helper('gspay')->__('The amount has been authorized and captured by Gspay.');
                    $this->_processSale($params['status'], $msg);
                    break;
            }
            return $msg;
    ...
    public function cancelEvent() {
        try {
            ...
            return Mage::helper('gspay')->__('The order has been canceled.');
        } catch (Mage_Core_Exception $e) {
            return $e->getMessage();
        } catch(Exception $e) {
            Mage::logException($e);
    ...
    protected function _validateEventData($fullCheck = true)
    {
        ...
        if (0 !== strpos($this->_order->getPayment()->getMethodInstance()->getCode(), 'gspay_')) {
        ...
        if ($fullCheck) {
            ...
            foreach ($checkParams as $key) {
                if ($key == 'merchant_id') {
                    ...
                    $md5String .= Mage::getStoreConfig(Gspay_Gspay_Helper_Data::XML_PATH_EMAIL, $this->_order->getStoreId()
                    );
                } elseif ($key == 'secret') {
                    $secretKey = Mage::getStoreConfig(
                        Gspay_Gspay_Helper_Data::XML_PATH_SECRET_KEY,
