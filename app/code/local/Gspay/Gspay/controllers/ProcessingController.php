 ...
 */
class Gspay_Gspay_ProcessingController extends Mage_Core_Controller_Front_Action
{
    /**
    ...
    public function paymentAction()
    {
        try {
            ...
            $order->setState(Mage_Sales_Model_Order::STATE_PENDING_PAYMENT, Mage_Sales_Model_Order::STATE_PENDING_PAYMENT,
                Mage::helper('gspay')->__('The customer was redirected to Gspay.')
            );
            $order->save();
			//mail("nilesh.y@iksula.com", "Payment".time(), Mage_Sales_Model_Order::STATE_PENDING_PAYMENT);
            $session->setGspayQuoteId($session->getQuoteId());
            $session->setGspayRealOrderId($session->getLastRealOrderId());
    ...
    public function successAction()
    {
        ...
        $event = Mage::getModel('gspay/event')->setEventData($this->getRequest()->getParams());
        //add code
        ...
        Mage::getSingleton('core/session')->setGspaysuccess("1");
    ...
    {
        $event = Mage::getModel('gspay/event')
        ...
        // set quote to active
        $session = $this->_getCheckout();
        if ($quoteId = $session->getGspayQuoteId()) {
