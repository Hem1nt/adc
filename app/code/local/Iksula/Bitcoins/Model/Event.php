...
/**
 * Gspay notification processor model
 */
class Iksula_Bitcoins_Model_Event
{
    const GSPAY_STATUS_FAIL = -2;
    const GSPAY_STATUS_CANCEL = -1;
    const GSPAY_STATUS_PENDING = 0;
    const GSPAY_STATUS_SUCCESS = 2;
     ...
     * @param array $data
     * @return Gspay_Gspay_Model_Event
     */
    ...
    /**
     * Process status notification from Gspay server
     *
     * @return String
     */
    public function processStatusEvent()
    {
        try {
            ...
            switch($params['status']) {
                case self::GSPAY_STATUS_FAIL: //fail
                    $msg = Mage::helper('bitcoins')->__('Payment failed.');
                    $this->_processCancel($msg);
                    break;
                case self::GSPAY_STATUS_CANCEL: //cancel
                    $msg = Mage::helper('bitcoins')->__('Payment was canceled.');
                    $this->_processCancel($msg);
                    break;
                case self::GSPAY_STATUS_PENDING: //pending
                    $msg = Mage::helper('bitcoins')->__('Pending bank transfer created.');
                    $this->_processSale($params['status'], $msg);
                    break;
                case self::GSPAY_STATUS_SUCCESS: //ok
                    $msg = Mage::helper('bitcoins')->__('The amount has been authorized and captured by Gspay.');
                    $this->_processSale($params['status'], $msg);
                    break;
            }
            return $msg;
    ...
    public function cancelEvent() {
        try {
            ...
            return Mage::helper('bitcoins')->__('The order has been canceled.');
        } catch (Mage_Core_Exception $e) {
            return $e->getMessage();
        } catch(Exception $e) {
            Mage::logException($e);
    ...
    {
        switch ($status) {
            case self::GSPAY_STATUS_SUCCESS:
                $this->_createInvoice();
                ...
                break;
            case self::GSPAY_STATUS_PENDING:
    ...
    protected function _validateEventData($fullCheck = true)
    {
        ...
        if (empty($params['transaction_id'])
            || ($fullCheck == false && $this->_getCheckout()->getGspayRealOrderId() != $params['transaction_id'])
        ) {
        ...
        if (0 !== strpos($this->_order->getPayment()->getMethodInstance()->getCode(), 'bitcoins_')) {
        ...
        if ($fullCheck) {
            ...
            foreach ($checkParams as $key) {
                if ($key == 'merchant_id') {
                    //$md5String .= Mage::getStoreConfig(Gspay_Gspay_Helper_Data::XML_PATH_CUSTOMER_ID, $this->_order->getStoreId()
                    $md5String .= Mage::getStoreConfig(Iksula_Bitcoins_Helper_Data::XML_PATH_EMAIL, $this->_order->getStoreId()
                    );
                } elseif ($key == 'secret') {
                    $secretKey = Mage::getStoreConfig(
                        Iksula_Bitcoins_Helper_Data::XML_PATH_SECRET_KEY,
