...
class Iksula_Callforoffer_Model_Observer
{
	   ...
	   //          }
		       ...
		       // $order->setCallforoffers($callforfreeval);
				...
				$order->setData('timetocall',$timetocall);
				$order->save();
        ...
        //set value for the call for offers in customer section
				...
				$cheetahIds = explode("#", Mage::getModel('core/variable')->loadByCode('cheetah_place_order')->getValue('plain'));
				$eid = $cheetahIds[0];
				$aid = $cheetahIds[1];
				$callOfferModel = Mage::getModel('callforoffer/callforoffers');
				$customerName = $callOfferModel->getUserName($email);
				$callOfferModel->CheetaApi($order->getCustomerEmail(), $eid, $aid, $customerName['firstname'], $customerName['lastname']);
