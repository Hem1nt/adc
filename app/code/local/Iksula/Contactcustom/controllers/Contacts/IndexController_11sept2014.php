        ...
        if ( $post ) {
            ...
            try {
                ...
                */
                ...
                // $email="cs@alldaychemist.com";
                $email="david@alldaychemist.com";
                if($post['querytype']=='Request a Call Back'){
                    $sensbccMail = true;
                    $email="vikas@derricwood.com";
                ...
                $vars = array('querytype' => $post['querytype'], 'timetocall' => $timetocall,'productname' => $post['productname'],'custname' => $post['name'], 'custemail' => $post['email'], 'telephone' => $post['telephone'], 'ordernumber' => $post['ordernumber'], 'comment' => $post['comment']);
  				$mailTemplate = Mage::getModel('core/email_template')->load('25');
				$mailTemplate->setTemplateSubject($post['querytype'])->save();
                if($post['querytype']=='Request a Call Back'){
                    $emailcopy = 'dw.amd@motifinc.com';
                    Mage::log($emailcopy.'email send to this id',null,'contactus_request.log');
