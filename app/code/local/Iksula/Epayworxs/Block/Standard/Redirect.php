...
/**
 * Redirect to epayworxs
 *
 * @category    Mage
 * @package     Iksula_Epayworxs
 * @name        Iksula_Epayworxs_Block_Standard_Redirect
 * @author      Magento Core Team <core@magentocommerce.com>
 */

class Iksula_Epayworxs_Block_Standard_Redirect extends Mage_Core_Block_Abstract
{

    protected function _toHtml()
    {
        $standard = Mage::getModel('epayworxs/standard');
        $form = new Varien_Data_Form();
        $form->setAction($standard->getepayworxsUrl())
            ->setId('epayworxs_standard_checkout')
            ->setName('epayworxs_standard_checkout')
            ->setMethod('POST')
		...
		$mode=Mage::getSingleton('epayworxs/config')->getTransactionMode();
		...
		$hash = Mage::getSingleton('epayworxs/config')->getSecretKey()."|".Mage::getSingleton('epayworxs/config')->getAccountId()."|".$amount."|".$referenceno."|".$returnurl."|".$mode;
       ...
       */
        ...
        $html = '<html><body>';
        $html.= $this->__('You will be redirected to E-Payworxs Solutions in a few seconds.');
        $html.= $form->toHtml();
		$html .= '<style type="text/css">#epayworxs_standard_checkout .submit{display:none;}</style>';
		$html.= '<script type="text/javascript">
				epayworxs_standard_checkout.submit.click();
		</script>';
        $html.= '</body></html>';
