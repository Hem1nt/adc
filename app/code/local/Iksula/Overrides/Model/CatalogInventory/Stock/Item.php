<?php
class Iksula_Overrides_Model_CatalogInventory_Stock_Item extends Mage_CatalogInventory_Model_Stock_Item
{
	/**
    ...
    public function checkQuoteItemQty($qty, $summaryQty, $origQty = 0)
    {
        ...
        if ($this->getMaxSaleQty() && $qty > $this->getMaxSaleQty()) {
            $result->setHasError(true)
                ->setMessage(
                    Mage::helper('cataloginventory')->__('The maximum quantity allowed for purchase is %s. Please select a different pack size.', $this->getMaxSaleQty() * 1)
                )
