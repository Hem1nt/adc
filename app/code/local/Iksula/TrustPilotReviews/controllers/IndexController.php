      ...
      if($nextpage_apiurl){
        ...
        if(array_key_exists('reviews',$reviewsCollection)){
          ...
          foreach ($reviewsCollection['reviews'] as $reviews){
            ...
            $html .=  "<div class='review_txt'>".ucfirst(strtolower($reviews['text'])).'</div>';
            $html .=  '</div>';
            if($reviews['companyReply']['text'] != ''):$html .=  "<div class='companyReply'><div style='color:red;font-weight: normal;
            color: #506f00;font-weight: 600;'>AllDayChemist Reply </div><br>".ucfirst(strtolower($reviews['companyReply']['text'])).'</div>';endif;
            $html .=  '</div>';
          }
