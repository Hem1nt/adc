...
class Kanavan_Searchautocomplete_Block_Suggest extends Mage_Core_Block_Template
{
	public function _prepareLayout()
    {
     ...
     public function getSuggestProducts()     
     {
		...
		//$results->addAttributeToFilter('name',array('like' => Mage::app()->getRequest()->getParam('q') . '%'));
		 $results->addAttributeToSelect(array('name','us_brand_name','generic_name'));
        $results->addFieldToFilter(array (
                array('attribute'=>'name', 'like' => trim('%'.Mage::app()->getRequest()->getParam('q')) . '%'),
                array('attribute'=>'us_brand_name', 'like' => trim(Mage::app()->getRequest()->getParam('q')) . '%'),
				array('attribute'=>'generic_name', 'like' => trim(Mage::app()->getRequest()->getParam('q')) . '%'),
            ));
		//echo "<pre>";
		//echo $results->getSelect();exit;
        $results->addAttributeToFilter('visibility', array('neq' => 1));
        $results->addAttributeToFilter('status', array('eq' => 1));
