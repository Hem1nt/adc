	...
	static public function getTransactionDetail($type, $detail)
    {
    	...
    	switch($type)
    	{
    		...
    		case self::BUY_PRODUCT:
    			$order = Mage::getModel("sales/order")->loadByIncrementId($detail);
 				$result = Mage::helper('credit')->__("Commission of order: #%s",$detail);
    			break;
    		case self::REFUND_PRODUCT_AFFILIATE:
    			$order = Mage::getModel("sales/order")->loadByIncrementId($detail);
    			$result = Mage::helper('credit')->__("Commission refund of order: #%s",$detail);
    			break;
    		case self::WITHDRAWN:
    			$withdrawn = Mage::getModel("affiliate/affiliatewithdrawn")->load($detail);
    			$result = Mage::helper('credit')->__("Withdrawal money: %s (Taken fee: %s)",Mage::helper('core')->currency($withdrawn->getWithdrawnAmount(),true,false),Mage::helper('core')->currency($withdrawn->getFee(),true,false));
    			break;
    		case self::CANCEL_WITHDRAWN:
    			$withdrawn = Mage::getModel("affiliate/affiliatewithdrawn")->load($detail);
    			$result = Mage::helper('credit')->__("Canceled withdrawal money: %s (Taken fee: %s)",Mage::helper('core')->currency($withdrawn->getWithdrawnAmount(),true,false),Mage::helper('core')->currency($withdrawn->getFee(),true,false));
    			break;
    	}
    	return $result;
    }
