...
class Mageplace_Freshdesk_TicketController extends Mage_Core_Controller_Front_Action
{
    ...
    public function saveAction()
    {
        try {
            ...
            $ticketType = array('General Enquiries','Sales Enquiries','Order Cancellations (within 12 hours of ordering)','Order Status (3-4 days after order is placed)','Payment Status','Billing Department (wrong charges on CC, etc.)','Address Change or Add to Cart (within 12 hours of ordering)','Shipping Department (rates, durations etc.)','Website (cart, lost password, broken links, etc.)','Refunds (order never arrived, wrong item received etc.)','Price Matching','Request a Call Back','Affiliate Program','Phone orders (Representative will contact you in 24 working hours)','Replacement');
            if(in_array($post['ticket_type'],$ticketType)){
                $post['product_name_298649'] = 'NA';
            }
            if (empty($post)) {
