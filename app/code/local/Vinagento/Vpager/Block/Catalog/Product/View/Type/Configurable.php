...
class Vinagento_Vpager_Block_Catalog_Product_View_Type_Configurable extends Mage_Catalog_Block_Product_View_Type_Configurable
{
    public function getJsonConfig()
    {
        ...
        foreach ($this->getAllowProducts() as $product) {
            ...
            $childProductPrice[$productId] = $product->getPrice();
            //$childProductPrice[$productId][] = $product->getPrice();
            //echo '<p style="display:none">'.$productId.'</p>';
                ...
                foreach ($prices as $value) {
                    ...
                    if($bonus != "")
					{
						$info['options'][] = array(
							...
							'label'     => $value['label'] ." + ".$value['label']." ".$pharma_form." Free - US$ ".number_format($childProductPrice[$productsIndex[0]], 2, '.', ''),
					...
					else
					{
						$info['options'][] = array(
							'id'        => $value['value_index']."-".$productsIndex[0],
							'label'     => $value['label'] ." ". $pharma_form." - US$ ".number_format($childProductPrice[$productsIndex[0]], 2, '.', ''),
							'price'     => $configurablePrice,
							'oldPrice'  => $this->_prepareOldPrice($value['pricing_value'], $value['is_percent']),
							'products'  => $productsIndex,
						);
