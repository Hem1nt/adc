...
function wp_rp_generate_related_posts_list_items($related_posts, $selected_related_posts, $post_categories) {
	...
	for ($i = 0; $i < $limit; $i++) {
		...
		$img = wp_rp_get_post_thumbnail_img($related_post, $image_size);
		if ($img) {
			$output .=  '<div class="item"><div class="related_img_wrap"><a href="' . $post_url . '" class="wp_rp_thumbnail">' . $img . '</a></div>';
		}
		...
		$output .= '<a href="' . $post_url . '" class="wp_rp_title">' . wptexturize($related_post->post_title) . '</a></div>';
...
function wp_rp_get_related_posts($before_title = '', $after_title = '') {
	...
	$output = '<div class="wp_rp_wrap scroll_available' . $css_classes_wrap . '" ' . $first_id_attr . $wrap_style . '>' .
			'<div class="related_content_wrapper">
				<div class="related_content">
				<span class="related_text">related</span>
				<div class="toggle_img"></div>
