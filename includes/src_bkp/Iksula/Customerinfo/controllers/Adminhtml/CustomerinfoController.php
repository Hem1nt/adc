...
class Iksula_Customerinfo_Adminhtml_CustomerinfoController extends Mage_Adminhtml_Controller_Action
{
		...
		{			    
				...
				$model = Mage::getModel("customerinfo/customerinfo")->load($id);
				if ($model->getId()) {
					Mage::register("customerinfo_data", $model);
					$this->loadLayout();
					$this->_setActiveMenu("customerinfo/customerinfo");
					...
					$this->_addContent($this->getLayout()->createBlock("customerinfo/adminhtml_customerinfo_edit"))->_addLeft($this->getLayout()->createBlock("customerinfo/adminhtml_customerinfo_edit_tabs"));
					$this->renderLayout();
				} 
				else {
					Mage::getSingleton("adminhtml/session")->addError(Mage::helper("customerinfo")->__("Item does not exist."));
					$this->_redirect("*/*/");
				}
		}
			...
			$post_data=$this->getRequest()->getPost();
				...
				if ($post_data) {
					...
					try {
						...
						$model = Mage::getModel("customerinfo/customerinfo")
						->addData($post_data)
		...
		public function exportCsvAction()
		{
			...
			$grid       = $this->getLayout()->createBlock('customerinfo/adminhtml_customerinfo_grid');
